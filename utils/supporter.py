...
def generate_error_description(client, customer_name, process_name, point_of_failure, steps_log, screenshot):
    messages = [
        ...
        },
        {
            ...
            "content": [
                {"type": "text", "text": (
                    "Complete the mrkdwn text within the JSON object below. Return the entire JSON object only. Ensure to start with a (short) objective description of the error under investigation, clearly describing at which step, which loop, and in which task the error occured (also indicate if it is in the first loop). But do not just repeat the input sentence. Subtly add layout/formatting to the output JSON by including markdown formatted text or indicating emojis with enclosed ':' signs.\n"
                    "```json\n"
                    "{{\n"
                    "  \"type\": \"section\",\n"
                    "  \"text\": {{\n"
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "- Changes in systems (e.g., missing elements, changed xpaths)\n"
                "- Typical RPA and automation errors (e.g., UI automation issues, API failures)\n"
                "- Unexpected pop-ups or dialogues disrupting the workflow (e.g., confirmation pop-ups, error dialogues)\n\n"
                "Audience:\n"
                "Your target audience is the employees of Yarado who provide support to processes running into problems. Errors pop up in Slack, triggering the Yarado-supporter (the name of the AI model) to provide support. The output will also be sent in Slack.\n\n"
                ...
                "Output Format:\n"
                "You should provide the cause analysis in the form of a JSON object. Return the entire JSON object only. Subtly add layout/formatting to the output JSON by including markdown formatted text or indicating emojis with enclosed ':' signs.  However, keep in mind not to repeat earlier generated content (especially the content given in 'the objective error description generated by the AI'.\n"
                "```json\n"
                "{{\n"
                "  \"type\": \"section\",\n"
                "  \"text\": {{\n"
